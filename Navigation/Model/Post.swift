...
class Posts{
  ...
  private  var posts: [Post] = [Post]()
    ...
    static func createMockData() ->[Post]{
          var posts = [Post]()
        posts.append(Post(title: "Pарядка устройств по воздуху", author: "3DNews", description: "Ресурс Nikkei сообщает о том, что японская телекоммуникационная компания SoftBank в скором времени приступит к тестированию технологии, которая позволит использовать сотовые вышки в качестве станций беспроводной зарядки для носимых гаджетов", image: "news1", likes: 10, views: 22))
